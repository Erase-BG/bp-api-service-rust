pub mod services {
    ...
    ///
    pub async fn send_message(task_group: &Uuid, ws_sessions: Arc<Mutex<HashMap<String, Session>>>,
                              response: Value) {
    ...
    pub mod task {
        ...
        pub async fn send_internal_server_error(instance: &BackgroundRemoverTask,
            ...
            if let Some(session) = ws_session {
                match session.text(response.to_string()).await {
                    Ok(_) => {
    ...
    pub mod task_ws {
        ...
        use crate::AppData;
        use crate::implementations::services::{build_standard_response, send_message};
        ...
        ///
        pub async fn listen_ws_message(data: web::Data<AppData>, task_group: Uuid, ws_session: Session,
            ...
            let ws_sessions_set_ref = ws_sessions.clone();
            {
                ...
                sessions_map.insert(task_group.to_string(), ws_session);
            }

            let sessions_ref_messages = ws_sessions.clone();

            // Listen incoming messages from ws stream
            while let Some(result) = message_stream.recv().await {
                match result {
                        ...
                        match message {
                            Message::Text(message) => {
                                handle_received_message(
                                    data.db_wrapper.clone(),
                                    data.tx_image_mpsc_channel.clone(),
                                    &task_group,
                            ...
                            Message::Close(close_reason) => {
                                log::error!("WebSocket connection closed. Reason: {:?}", close_reason);

                                // Removing session from ws sessions
                                let mut sessions = sessions_ref_messages.lock().await;
                                sessions.remove(&task_group.to_string());
                            }
                            _ => {
                                // Removing session from ws sessions
                    ...
                    Err(error) => {
                        log::error!("Removing session from ws_sessions.");
                        log::error!("Failed to read websocket message stream. Error: {}", error);
